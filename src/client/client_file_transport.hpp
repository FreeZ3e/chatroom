...
class file_transport : private transport_base
{
	...
	public:
		...
		int send_wrapper(const SOCKET& server_socket , const string& path)
		{
			string name = path.substr(path.find_last_of("/") , path.size());

			if (!send_signal(server_socket))
				return 1;
	...
	private:
		bool send_signal(const SOCKET& server_socket)
		{
			const string signal = "/filerc";
			return send(server_socket , signal.c_str() , signal.size() + 1 , 0) >= 0;
		}

		bool send_filename(const SOCKET& server_socket , const string& name)
		{
			return send(server_socket , name.c_str() , name.size() + 1 , 0) > 0;
		}

		bool recv_filename(const SOCKET& server_socket)
		{
			char filename[256];
			int len = recv(server_socket , filename , 256 , 0);
		...
		bool send_file(const SOCKET& server_socket , const string& path)
		{
			...
			while (!file.eof())
			{
				...
				int len = file.gcount();
				if (send(server_socket , file_buf , len , 0) < 0) //send file
					return false;
			}
		...
		bool recv_file(const SOCKET& server_socket)
		{
			char file_buf[1024] = { 0 };
			ofstream file;
			...
			while (1)
			{
				int len = recv(server_socket , file_buf , 1024 , 0);
				if (len > 0)
